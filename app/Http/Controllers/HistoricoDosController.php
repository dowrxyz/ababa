...
use App\Models\HistoricoDos;
use Illuminate\Support\Facades\Storage;

class HistoricoDosController extends Controller
{
    ...
    public function socios_tipo($valor)
    {
        ...
        return redirect()->route("historicodos");
    }
    ...
    public function show(Request $request)
    {
        ...
        if (Auth::user()->tipo_socio == 1) {
            $socio = HistoricoDos::with("banco", "creador")
                ->where("convenio", 1)
                ->where("estado", 1)
                ->orderBy("id", "DESC")
                ->paginate(25);
    ...
    public function buscarSocio(Request $request)
    {
        ...
        //por convenio
        if (Auth::user()->tipo_socio == 1) {
            $query = HistoricoDos::with("banco", "creador")
                ->where("convenio", 1)
                ->where("estado", 1);
        ...
        //inactivos
        if (Auth::user()->tipo_socio == 3) {
            $query = HistoricoDos::with("banco", "creador")->where("estado", 2);
        ...
        //activos
        if (Auth::user()->tipo_socio == 5) {
            $query = HistoricoDos::with("banco", "creador")->where("estado", 1);
        ...
        //BLOQUEADOS
        if (Auth::user()->tipo_socio == 7) {
            $query = HistoricoDos::with("banco", "creador")->where("estado", 3);
        ...
        //Fallecidos
        if (Auth::user()->tipo_socio == 8) {
            $query = HistoricoDos::with("banco", "creador")->where("estado", 4);
    ...
    {
        return HistoricoDos::where("id", $socio)
            ->with("banco")
            ->first();
    }
    ...
    {
        $usuario = Auth::user();
        $socio = HistoricoDos::find($usuario->socio_id);
